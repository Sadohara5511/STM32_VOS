...
- カーネル初期化：[API] vosKernelInit関数
    カーネル変数や各種機能変数を初期化する。
- カーネルスタート：[API] vosKernelStart関数
    `実行可能状態`タスクを`実行状態`タスクへ遷移する。もし`実行可能状態`タスクがない場合、即リターンする。
- タスク・ディスパッチャー：[外部I/F] vosTaskDispatch関数
    詳細仕様未定
- コンテキストスイッチ：[外部I/F] vosPendSVHandler 割り込みハンドラ
    タスク切り替え。
- システムタイマ：[外部I/F] vosSysTickHandler 割り込みハンドラ
    仕様未定
...
- タスク生成：[API] vosTaskCreate関数
    引数は、タスク関数、スタックサイズ、スタック領域（、スタート指示）である。
    タスクコントロールブロックを獲得して情報をセットする。獲得したTaskCBをタスクハンドルとして返す。
- タスクスタート：[API] vosTaskStart関数
    引数は、タスクハンドルである。
    タスクハンドルのTaskCBをREADYキューに繋げる。
- タスクストップ：[API] vosTaskStop関数
    引数は、タスクハンドルである。
    タスクハンドルのTaskCBをキューから外す。
- タスク終了：[API] vosTaskExit関数
    引数なし。自タスクを終了する。
...
- メッセージキュー生成：[API] vosMsgQueCreate関数
    引数は、メッセージキュー数、メッセージデータサイズ、メッセージデータ領域（メッセージプール領域）である。
    メッセージコントロールブロック(MsgCB)を獲得して情報をセットする。獲得したMsgCBをメッセージキューハンドルとして返す。
- メッセージ送信：[API] vosMsgSend関数
    引数は、メッセージキューハンドル、ユーザーが用意したメッセージデータまたはvosMsgBufferGet関数で得たメッセージバッファである。
    ユーザーが用意したメッセージデータが渡された場合、メッセージプール領域にメッセージデータがコピーされ保持される。
- メッセージ受信：[API] vosMsgReceive関数
    引数は、メッセージキューハンドルである。受信したメッセージデータのポインタを返す。
    受信したメッセージデータは、vosMsgBufferRelease関数がコールされるまでメッセージプール領域に保持される（データ保証する）。
- メッセージバッファ獲得：[API] vosMsgBufferGet関数
    引数は、メッセージキューハンドルである。獲得したメッセージバッファを返す。
    ユーザーは、返されたメッセージバッファにデータをセットしてメッセージ送信する。
- メッセージバッファ解放：[API] vosMsgBufferRelease関数
    引数は、メッセージキューハンドル、vosMsgBufferGet関数で得て解放するメッセージバッファである。
...
- イベントフラグ待ち：[API] vosEvtFlagWait関数
...
- イベントフラグ・セット：[API] vosEvtFlagPost関数
...
- イベントフラグ・クリア：[API] vosEvtFlagClear関数
...
### 3-5-2.データ設計
```
int32_t     g_CfriticalCounter;         /* 割り込み抑止解除カウンタ */
```
...
**プロトタイプ**
```
bool vosTaskStart(vodTaskHandle_t handle);
```
**パラメータ**
...
**補足説明**
    タスクハンドルのTaskCBをREADYキューに繋げ、カーネルがスタート状態ならRUNタスク優先度と比較し、自タスクが高優先の場合、ディスパッチャーをコールする。

## 4-2-3.タスクストップ
**プロトタイプ**
bool vosTaskStop(vodTaskHandle_t handle);
**パラメータ**
...
**プロトタイプ**
```
vosErr_e vosErrorGet();
```
**リターン**
    0:エラーなし
    0以外:[エラー一覧](#6-1エラー一覧)参照。
...
**パラメータ**
    ...
    ※１：NULLを指定するとメッセージプール領域が戻り値で返される。この場合、vosMsgBufferRelease関数をコールして、メッセージバッファを解放する必要がある。受信するメッセージバッファが指定された場合、メッセージプールからメッセージコピーが行われ、メッセージプールのデータ領域は自動的に解放される。
**リターン**
...
**補足説明**
    ユーザーは、返されたメッセージバッファにデータをセットしてメッセージ送信する。但し、データサイズは、メッセージキュー生成時に指定したサイズを超えてコピーしてはならない。VOSでは抑止できないためメモリ破壊が起こる。
...
**リターン**
    ...
    false   失敗（パラメータエラー）
**機能説明**
    該当するイベントフラグコントロールブロックの指定ビットをクリア'0'にする。
**補足説明**
    他タスクにディスパッチしないAPIである。
...
**機能説明**
    タスク切り替え割り込みハンドラ
    RUNキューのタスクは、タスク次遷移先がREADYキューまたはWAITキュー等の遷移先キューに繋げる。
    外部変数g_dispach_taskのタスクハンドルをRUNキューに繋げる。
**補足説明**
...
**機能説明**
    ...
    PendSV割り込みを発生させる。
**補足説明**
    タスク・ディスパッチの準備とは、RUNキューのタスクの次遷移先にREADYキューをセットする。READYキュー先頭のタスクを外し、外部変数g_dispach_taskにタスクハンドルをセットする。
