...
---
## 3-4.イベントフラグ機能
イベントフラグ機能は、タスク間で32ビットデータを送受信する機能である。32ビットのビットフィールドは、ユーザータスク間にて自由に取り決め、VOSはこれを関知しないがビットフィールドをセット／クリアする操作APIを提供する。
...
- イベントフラグ・セット：vosSetEvtFlg関数
    ...
    イベントフラグ待ちしているタスクが高優先の場合、そのタスクにディスパッチする。
